...
public class TypoProcessor
{
	...
	private HashSet<Typo> m_toPrepend;
	...
	public void prependHadithNumber(int pageNumber, int hadithNumber) {
		m_toPrepend.add( new Typo(pageNumber, decorateContent( String.valueOf(hadithNumber) ), null) );
	}
	...
	public void process(JSONObject json)
	{
		...
		for (Typo typo: m_toPrepend)
		{
			if (typo.pageNumber == pageNumber)
			{
				String content = typo.value+json.get("content").toString();
				json.replace("content", content);
			}
		}
		...
		for (StripperData data: m_toStrip)
		{
			if (data.pageNumber == pageNumber)
			{
				...
				String hadeeth = content.substring(start, end).replaceAll("<br />", "").replaceAll( decorateContent(data.inner), data.replacement );
